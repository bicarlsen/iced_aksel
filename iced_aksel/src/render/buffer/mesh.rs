...
use iced_core::{Point, Rectangle, Transformation};
...
/// Writes geometry directly into WebGPU vertex/index buffers.
struct MeshBufferWriter<'a> {
    vertices: &'a mut Vec<SolidVertex2D>,
    indices: &'a mut Vec<u32>,
    // Color packed as [f32; 4] or u32, matching your SolidVertex2D definition
    packed_color: Packed,

    // The index of the first vertex of the current shape (the "Pivot").
    start_index: u32,
}

impl<'a> GeometryWriter for MeshBufferWriter<'a> {
    fn move_to(&mut self, p: Point) {
        // Start a new shape. Capture the current index as our "Pivot".
        self.start_index = self.vertices.len() as u32;
    ...
    fn line_to(&mut self, p: Point) {
        ...
        // 2. Analytic Triangulation (The "Fan" Logic)
        // If we have at least 3 points (Pivot, Previous, Current), we can form a triangle.
        // start_index is Pivot (A).
        // start_index + 1 is the first point after pivot.
        // So if current_index > start_index + 1, we have enough points.
        if current_index > self.start_index + 1 {
            self.indices.push(self.start_index); // A (Pivot)
            self.indices.push(current_index - 1); // B (Previous)
            self.indices.push(current_index); // C (Current)
        }
    }

    fn close(&mut self) {
        // No-Op for Filled Triangle Fans.
        // The triangles generated by line_to already cover the surface area
        // because the fan naturally connects p0 -> pLast -> pCurrent.
        // Explicit closing is only strictly needed for outlines/strokes,
        // which use a different logic usually. For filled convex shapes, this is fine.
    }
}
...
impl MeshBatcher {
    ...
    pub fn add_primitive(&mut self, primitive: Primitive) {
        match primitive {
            Primitive::Rectangle { xy1, xy2, fill, .. } => {
                if let Some(color) = fill {
                    // 1. Prepare Data
                    let packed_color = iced_graphics::color::pack(color);
                    let rect_shape = RectangleGeometry::new(xy1, xy2);

                    // 2. Initialize & Access Buffer Safely
                    let mesh = self.data.get_mesh_mut();

                    // 3. Create Writer (Splitting the borrow)
                    // We borrow vertices and indices separately from the mutable mesh ref
                    let mut writer = MeshBufferWriter {
                        vertices: &mut mesh.vertices,
                        indices: &mut mesh.indices,
                        packed_color,
                        start_index: 0, // Will be set by move_to
                    };
